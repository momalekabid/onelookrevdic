...
class JSONDataset(Dataset):
    ...
    def __init__(
        ...
        for json_dict in self.items:
            ...
            for arch in SUPPORTED_ARCHS:
                if arch in json_dict:
                    ...
                    #print(f"{arch} found in json_Dict")
                    json_dict[f"{arch}_tensor"] = torch.tensor(json_dict[arch])
            if "bert" in json_dict:
                json_dict["bert_tensor"] = torch.tensor(json_dict["bert"])
        if self.use_spm:
            self.vocab = {
        ...
        self.has_bert = "bert" in self.items[0]
...
def get_dataloader(dataset, batch_size=1024, shuffle=True):
    ...
    def do_collate(json_dicts):
        ...
        if has_vecs:
            batch["sgns_tensor"] = torch.stack(batch["sgns_tensor"])
        if has_bert:
            batch["bert_tensor"] = torch.stack(batch["bert_tensor"])
        return dict(batch)
